...
export default {
  ...
  methods: {
    ...
    emitSelect (value) {
      console.log(value);
      let data = value;
      value = value.text.replace(/<[\/]?strong>/gm, '')
      this.selected = true
      this.query = value
     
      //this.onSelect(value, this.name)
        this.onSelect(data, this.name)
    },
    getMatches (query) {
      if (query) {
        let matches = []
        ...
        this.source.forEach(value => {
          if (typeof value.text !== 'string') new TypeError(`Typeahead sources must be string. Received ${typeof value.text}.`)
          if (matches.length === this.limit) return
          let regexProps = regex.exec(value.text)
          if (regexProps) {
            isMatch = true
            let substr1 = value.text.substring(0, regexProps.index)
            let substr2 = `<strong>${value.text.slice(regexProps.index, regexProps.index + query.length)}</strong>`
            let substr3 = value.text.substring(regexProps.index + query.length)
            let match = substr1 + substr2 + substr3
            let id = value.value;
            matches.push(match)
            matchesData.push({ text: match, value: id})
